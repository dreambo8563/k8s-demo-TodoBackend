...
// RPCLoginHandler - handler login api
func RPCLoginHandler(c *gin.Context) {
	tracer := tracing.Tracer
	span := tracer.StartSpan("RPCLoginHandler")
	defer span.Finish()
	type LoginReq struct {
		User     string `json:"username"  binding:"required"`
		Password string `json:"password" binding:"required"`
	...
	if err := c.ShouldBindJSON(&login); err != nil {
		span.LogKV("event", "bind", "err", err)
		log.Sugar().Error("RPCLoginHandler", "receive params err", login)
		res.Err(c, err.Error())
		return
	}
	user := models.User{
	...
	user.NewUID(ctx)
	log.Info("new uid", zap.String("uid", user.ID))
	err := user.RPCNewToken(ctx)
	if err != nil {
		log.Error("new token", zap.String("err", err.Error()))
		res.Err(c, err.Error())
		return
	}
	log.Sugar().Infow("RPCLoginHandler res", "userInfo", user)
	res.JSON(c, &user)
}
